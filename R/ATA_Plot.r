...
plot.ata <- function(x, fcol=4, flty = 2, flwd = 2, ...)
{
	...
	range_y <- max_y - min_y
	min_last <- trunc(min_y) - trunc(range_y * 0.10)
	max_last <- trunc(max_y) + trunc(range_y * 0.10)
	...
	if (x$is.season==FALSE){
		...
		axis(side=2,at=seq(min_last, max_last,(range_last/10)), labels=seq(min_last, max_last,trunc(range_last/10)), las=1, lwd=1)
		...
		par(mar = c(bottom=2, 4.1, top=2, 1.1))
		plot(x$seasonal.adjusted,ylab="seasonal adjusted")
	}
	par(par.default)
}